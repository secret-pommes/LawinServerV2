...
app.get("/fortnite/api/cloudstorage/system/:file", (req, res) => {
  ...
  // yeah this LawinServerV2 uses Neonite's XMPP service at builds that are below v3.5
  // TCP Cloudstorage (TCP xmpp clients)
  if (memory.build < "3.5") {
    file = path.join(__dirname, "..", "TCP_CloudStorage", req.params.file); // <v3.5
  } else if (memory.season > 3) {
    file = path.join(__dirname, "..", "CloudStorage", req.params.file); // Season 3 - 2
  } else if (req.headers["user-agent"].includes("Cert")){
    file = path.join(__dirname, "..", "CloudStorage", req.params.file); // Cert Release
  } else {
    file = path.join(__dirname, "..", "CloudStorage", req.params.file); // websocket (v3.5+)
  }
  // normal cloudstorage (websocket xmpp clients)
...
app.get("/fortnite/api/cloudstorage/user/*/:file", verifyToken, (req, res) => {
  ...
  let file = path.join(
    clientSettingsPath,
    `ClientSettings-${memory.season}.Sav`
  );
...
function getRawBody(req, res, next) {
  ...
  try {
    ...
    res.status(400).json({
      error: "Something went wrong while trying to access the request body.",
    });
  }
}
