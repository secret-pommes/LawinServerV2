...
app.get("/account/api/oauth/exchange", verifyToken, (req, res) => {
  return res.status(400).json({
    error:
      "This endpoint is deprecated, please use the discord bot to generate an exchange code.",
  });
  // remove the return code above if you still want to make use of this endpoint

  let token = req.headers["authorization"].replace("bearer ", "");
  const exchange_code = functions.MakeID().replace(/-/gi, "");
  ...
  setTimeout(() => {
    let exchangeCode = global.exchangeCodes.findIndex(
      (i) => i.exchange_code == exchange_code
    );
