...
app.get(
  ...
  async (req, res) => {
    let user = await User.findOne({
      username_lower: req.params.displayName.toLowerCase(),
      banned: false,
    }).lean();
    if (!user)
...
app.get("/persona/api/public/account/lookup", async (req, res) => {
  ...
  let user = await User.findOne({
    username_lower: req.query.q.toLowerCase(),
    banned: false,
  }).lean();
  if (!user)
    return error.createError(
...
app.get("/api/v1/search/:accountId", async (req, res) => {
  ...
  let users = await User.find({
    username_lower: new RegExp(`^${req.query.prefix.toLowerCase()}`),
    banned: false,
  }).lean();
  ...
  for (let user of users) {
    ...
    response.push({
      ...
      matchType:
        req.query.prefix.toLowerCase() == user.username_lower
          ? "exact"
          : "prefix",
      epicMutuals: 0,
...
app.get("/account/api/public/account/:accountId", verifyToken, (req, res) => {
  ...
  log.backend(
    `${req.user.username} logged in | AccountId: ${req.user.accountId}`
  );
});
