...
app.all(
  "/friends/api/v1/*/friends/:friendId/alias",
  verifyToken,
  getRawBody,
  async (req, res) => {
    ...
    const friendIndex = friends.list.accepted.findIndex(
      (i) => i.accountId == req.params.friendId
    );
...
app.post(
  "/friends/api/*/friends*/:receiverId",
  verifyToken,
  async (req, res) => {
    ...
    if (sender.list.incoming.find((i) => i.accountId == receiver.accountId)) {
      if (
        !(await friendManager.acceptFriendReq(
          sender.accountId,
          receiver.accountId
        ))
      )
      ...
      !sender.list.outgoing.find((i) => i.accountId == receiver.accountId)
    ) {
      if (
        !(await friendManager.sendFriendReq(
          sender.accountId,
          receiver.accountId
        ))
      )
...
app.delete(
  ...
  async (req, res) => {
    ...
    if (
      !(await friendManager.deleteFriend(sender.accountId, receiver.accountId))
    )
      return res.status(403).end();
...
app.post(
  "/friends/api/*/blocklist*/:receiverId",
  verifyToken,
  async (req, res) => {
    let sender = await Friends.findOne({ accountId: req.user.accountId });
    let receiver = await Friends.findOne({ accountId: req.params.receiverId });
    ...
    if (
      !(await friendManager.blockFriend(sender.accountId, receiver.accountId))
    )
      return res.status(403).end();
...
function getRawBody(req, res, next) {
  if (req.headers["content-length"]) {
    if (Number(req.headers["content-length"]) > 16)
      return res
        .status(403)
        .json({ error: "File size must be 16 bytes or less." });
  }
