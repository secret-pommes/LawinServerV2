    ...
    ws.on("message", async (message) => {
      ...
      switch (msg.root.name) {
        ...
        case "auth":
          ...
          if (!functions.DecodeBase64(msg.root.content).includes("\u0000"))
            return Error(ws);

          let decodedBase64 = functions
            .DecodeBase64(msg.root.content)
            .split("\u0000");

          let object = global.accessTokens.find(
            (i) => i.token == decodedBase64[2]
          );
          if (!object) return Error(ws);

          if (global.Clients.find((i) => i.accountId == object.accountId))
            return Error(ws);
        ...
        case "iq":
          ...
          switch (msg.root.attributes.id) {
            case "_xmpp_bind1":
              ...
              if (global.Clients.find((i) => i.accountId == accountId))
                return Error(ws);

              let findResource = msg.root.children
                .find((i) => i.name == "bind")
        ...
        case "message":
          ...
          switch (msg.root.attributes.type) {
            case "chat":
              ...
              let receiver = global.Clients.find(
            ...
            case "groupchat":
              ...
              MUC.members.forEach((member) => {
                let ClientData = global.Clients.find(
                ...
                ClientData.client.send(
                  XMLBuilder.create("message")
                    ...
                    .attribute(
                      "from",
                      getMUCmember(roomName, displayName, accountId, resource)
                    )
                    .attribute("xmlns", "jabber:client")
        ...
        case "presence":
            ...
            case "unavailable":
              ...
              if (
                ...
                msg.root.attributes.to
                  .split("/")[0]
                  .endsWith(`@muc.${global.xmppDomain}`)
              ) {
                ...
                let memberIndex = global.MUCs[roomName].members.findIndex(
            ...
            default:
              if (
                ...
                if (!global.MUCs[roomName])
                  global.MUCs[roomName] = { members: [] };

                if (
                  global.MUCs[roomName].members.find(
                    (i) => i.accountId == accountId
                  )
                )
                  return;
          ...
          let away = msg.root.children.find((i) => i.name == "show")
            ? true
            : false;
      ...
      global.Clients.forEach((ClientData) => {
        ...
        ClientData.client.send(
          XMLBuilder.create("message")
            .attribute(
              "id",
              functions.MakeID().replace(/-/gi, "").toUpperCase()
            )
    ...
    accepted.forEach((friend) => {
      ...
      if (ClientData.lastPresenceUpdate.away)
        xml = xml
          .element("show", "away")
          .up()
          .element("status", ClientData.lastPresenceUpdate.status)
          .up();
      else
        xml = xml.element("status", ClientData.lastPresenceUpdate.status).up();
...
function getMUCmember(roomName, displayName, accountId, resource) {
  return `${roomName}@muc.${global.xmppDomain}/${encodeURI(
    displayName
  )}:${accountId}:${resource}`;
}
